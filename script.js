...
function generatePDF() {
    ...
    const textOffset = pageWidth - 20 - textWidth;
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text(date, textOffset, 10);

    // Add Subject line centered
    const subjectText = 'Subject: NP Domain Registration';
    const subjectWidth = doc.getStringUnitWidth(subjectText) * doc.internal.getFontSize() / doc.internal.scaleFactor;
    const subjectOffset = (pageWidth - subjectWidth) / 2;
    doc.text(subjectText, subjectOffset, 80);
